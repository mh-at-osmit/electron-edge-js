var _fs = require('fs')
    , _path = require('./electronPath')
    , builtEdge = _path.resolve(__dirname, '../build/Release/' + (process.env.EDGE_USE_CORECLR || !_fs.existsSync(_path.resolve(__dirname, '../build/Release/edge_nativeclr.node')) ? 'edge_coreclr.node' : 'edge_nativeclr.node'))
    , edge;
...
function checkForPreCompiled() {
    ...
    if (process.versions.electron || process.env.ELECTRON_RUN_AS_NODE) {
        ...
        const preCompiledPath = _path.resolve(_path.join(__dirname, './native/', process.platform, process.arch, version, (process.env.EDGE_USE_CORECLR ? 'edge_coreclr' : 'edge_nativeclr')));
        if (_fs.existsSync(`${preCompiledPath}.node`)) {
...
else if (_fs.existsSync(builtEdge)) {
...
if (process.env.EDGE_USE_CORECLR && !process.env.EDGE_BOOTSTRAP_DIR && _fs.existsSync(_path.join(__dirname, 'bootstrap', 'bin', 'Release', 'netcoreapp1.1', 'bootstrap.dll'))) {
    process.env.EDGE_BOOTSTRAP_DIR = _path.join(__dirname, 'bootstrap', 'bin', 'Release', 'netcoreapp1.1');
}
...
exports.func = function (language, options) {
    ...
    if (typeof language !== 'string') {
        ...
        try {
            options.compiler = _path.normalizeElectronPath(compiler.getCompiler());
        }
        catch (e) {
        ...
        if (process.env.EDGE_USE_CORECLR) {
            var defaultManifest = _path.join(__dirname, 'bootstrap', 'bin', 'Release', 'netcoreapp1.1', 'bootstrap.deps.json');
            ...
            options.bootstrapDependencyManifest =
                compilerManifest && _fs.existsSync(compilerManifest)
                    ? compilerManifest
                    : defaultManifest;
        }
    }
